...
%ITERACION 
Ve(1)=0; Xf=[0;0;0;0]; u_k(1)=0; Xhatf=[0;0;0;0];
Xhat_a=[0;0;0;0]; X_a=[0;0;0;0];
X=zeros(4,round(tf/dt));    %X=[ia ; w ; tita ];
X(:,1)=[0,0,0,H_i];     %condiciones iniciales de X
%OBSERVADOR
Xhat=zeros(4,round(tf/dt)); %Xhat=[iahat ; wrhat ; titahat ]
...
% subplot(3,2,[5:6]);plot(t,u,'k');grid on;title('Acción de control');xlabel('Tiempo en Seg.');hold on;
% 
% modificar para tener 2 T distinto